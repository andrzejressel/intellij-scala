...
final class SbtProjectStructureImportingTest_ProdTestSourcesSeparatedEnabled extends SbtProjectStructureImportingLike {
  ...
  override protected def runTest(expected: project, singleContentRootModules: Boolean = false): Unit = {
    super.runTest(expected, singleContentRootModules)
  }
  ...
  def testSharedSources(): Unit = runTest(
    new project("sharedSourcesProject") {
      ...
      lazy val sharedSourcesModuleTest: module = new module("sharedSourcesProject.sharedSources-sources.test") {
        contentRoots := Seq(s"$getProjectPath/shared/src/test")
        libraryDependencies := scalaLibraries
      }
      ...
      lazy val fooTest: module = new module("sharedSourcesProject.foo.test") {
        ...
        moduleDependencies := Seq(
          new dependency(sharedSourcesModuleMain) { isExported := true },
          new dependency(fooMain) { isExported := false },
          new dependency(sharedSourcesModuleTest) { isExported := true }
        )
      }
      ...
      modules := Seq(
        ...
        foo, fooMain, fooTest,
        bar, barMain, barTest,
        sharedSourcesModule, sharedSourcesModuleMain, sharedSourcesModuleTest
      )
    }
  )
  ...
  def testSharedSourcesInsideMultiBuildProject(): Unit = runTest(
      new project("sharedSourcesInsideMultiBuildProject") {
        ...
        val sharedSourcesModuleInC1Test: module = new module("c1-sources.test", Array("c1")) {
          libraryDependencies := scalaLibraries
        }
       ...
       val fooMainInC1: module = new module("foo.main", Array("c1")) {
        ...
        val fooTestInC1: module = new module("foo.test", Array("c1")) {
          ...
          moduleDependencies ++= Seq(
            ...
            new dependency(sharedSourcesModuleInC1Test) { isExported := true },
        ...
        val c1Modules: Seq[module] = Seq(
          c1, c1Main, c1Test,
          sharedSourcesModuleInC1, sharedSourcesModuleInC1Main, sharedSourcesModuleInC1Test,
